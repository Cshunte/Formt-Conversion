...
src_txt_dir = "./Labelme_to_txt"
src_xml_dir = "./txt_to_xml"
...
for img in img_names:
    ...
    xml_file.write('    <folder>volley</folder>\n')
    xml_file.write('    <filename>' + str(img) + '.jpg' + '</filename>\n')
    xml_file.write('    <path>' + '/' + str(src_img_dir) + '/' + '.jpg' + '</path>\n')
    xml_file.write('    <source>\n')
    xml_file.write('        <database>Unspecified</database>\n')
    xml_file.write('    </source>\n')
    xml_file.write('    <size>\n')
    ...
    for img_each_label in gt:  # gt 包含txt全部内容，以每行内容为一个元素的列表
        ...
        imax = len(spt)
        for i in range(0, imax - 1, 30):

            # LQJ
            xml_file.write('    <object>\n')
            xml_file.write('        <name>' + str(spt[i + 1]) + '</name>\n')
            xml_file.write('        <pose>Unspecified</pose>\n')
            xml_file.write('        <truncated>0</truncated>\n')
            xml_file.write('        <difficult>0</difficult>\n')
            xml_file.write('        <bndbox>\n')
            xml_file.write('            <xmin>' + str(spt[i + 3]) + '</xmin>\n')
            xml_file.write('            <ymin>' + str(spt[i + 5]) + '</ymin>\n')
            xml_file.write('            <xmax>' + str(spt[i + 7]) + '</xmax>\n')
            xml_file.write('            <ymax>' + str(spt[i + 9]) + '</ymax>\n')
            xml_file.write('        </bndbox>\n')
            xml_file.write('    </object>\n')
